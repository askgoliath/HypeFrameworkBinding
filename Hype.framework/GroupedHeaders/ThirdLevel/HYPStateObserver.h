 ...
 * notification is issued. Services will only be started if explicitly told to.
 * @param hype The HYP singleton instance.
 ...
 * framework identifying that the cause of failure might not apply. Attempting
 * to start the framework's services is not guaranteed to succeed as other causes
 * for failure might exist, but they are likely to do so. It's up to the receiver
 * to decide whether the services should be started.
 * @param hype The HYP singleton instance.
 */
 ...
 * @discussion This notification is issued whenenever the Hype instance changes
 * state. This method could be used in alternative to -hypeDidStart: and
 * -hypeDidStop:error:, as it indicates when Hype enters into HYPStateRunning
 * and HYPStateIdle states. Whether to use this method or the other specific
 * notifications is a design call, as these notifications are guaranteed to
 * always be triggered when the state changes.
 * @param hype The HYP singleton instance.
 */
